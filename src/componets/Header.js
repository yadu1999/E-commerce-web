...
import { toggleDarkMode } from "../feautres/themeSlice";
...
function Header() {
  ...
  const userName = useSelector(selectUserName) ?? localStorage.getItem('name');
  ...
  return (
    <nav>
      ...
      <div className="logo">
        <img src={Logo} alt="image_logo" onClick={userName ? () => navigate("/") : navigate("/login") } />
      </div>
     {userName && <ul className="links">
        <li>
          {" "}
      ...
      <button
          ...
          className="fixed bottom-5 right-5 bg-blue-600 text-white p-2 rounded-full shadow-lg"
        >
          {darkMode ? "☀️" : "🌙"}
        </button>
    </nav>
